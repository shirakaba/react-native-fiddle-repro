...
function requireMetroVersion(requiredVersion) {
    const version = (0, metro_1.getMetroVersion)();
    if (!version) {
        return `Metro version ${requiredVersion} is required`;
    }
    if (!(0, version_1.greaterThanOrEqualTo)(version, requiredVersion)) {
 ...
 */
function TypeScriptPlugin(options, print) {
    if (options === false) {
        return () => void 0;
    }
    ...
    if (unsupportedMetroVersion) {
        (0, console_1.warn)(`TypeScriptPlugin disabled: ${unsupportedMetroVersion}`);
        return () => void 0;
    }
    const projectCache = (0, projectCache_1.createProjectCache)(print);
        ...
        if (platform) {
            ...
            for (const sourceFile of adds.concat(updates)) {
                ...
                if (projectInfo) {
                    ...
                    if (tssourceFiles.has((0, path_1.normalizePath)(sourceFile))) {
...
//# sourceMappingURL=serializerHook.js.map