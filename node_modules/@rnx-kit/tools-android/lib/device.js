...
export async function getDevices() {
    ...
    return stdout
        ...
        return {
            serial,
            state,
            description: Object.fromEntries(props.map((prop) => prop.split(":"))),
        };
    });
}
/**
 ...
 */
export async function launchEmulator(emulatorName) {
 ...
 */
export async function selectDevice(emulatorName, logger) {
    const attachedDevices = await getDevices();
    if (!emulatorName) {
        const physicalDevice = attachedDevices.find((device) => device.state === "device" && "usb" in device.description);
        if (physicalDevice) {
    ...
    // one the user wants.
    const attachedEmulator = attachedDevices.find((device) => device.state === "device" && !("usb" in device.description));
    if (attachedEmulator) {
