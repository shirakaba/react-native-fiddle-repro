...
function remoteSpecificBuildFlags({ isBuiltRemotely }, args) {
    if (isBuiltRemotely) {
        const NO_SANITIZE = "-fno-sanitize=undefined -fno-sanitize=bounds -fstack-protector-strong";
        args.push("CLANG_ADDRESS_SANITIZER=NO", "CLANG_UNDEFINED_BEHAVIOR_SANITIZER=NO", `OTHER_CFLAGS=$(inherited) ${NO_SANITIZE}`, `OTHER_LDFLAGS=$(inherited) ${NO_SANITIZE}`);
    }
    return args;
}
/**
 ...
 * to build it.
 */
export async function getBuildSettings(xcworkspace, params) {
    ...
    return buildSettings.find(({ buildSettings }) => buildSettings.WRAPPER_EXTENSION === "app");
}
/**
 * Returns device platform identifier for specified platform and destination.
 */
export function getDevicePlatformIdentifier(buildParams) {
 ...
 */
export async function parsePlist(app) {
 ...
 */
export function xcodebuild(xcworkspace, params, log = console.log) {
    ...
    args.push("-configuration", configuration, "-derivedDataPath", path.join(path.dirname(xcworkspace), "DerivedData"));
