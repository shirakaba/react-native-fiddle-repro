 ...
 */
export async function getAvailableSimulators(search = "iPhone") {
...
function pickSimulator(simulators) {
 ...
 */
export async function bootSimulator(simulator) {
...
function findSimulator(devices, deviceName, platformIdentifier) {
    if (deviceName) {
        return devices.find(({ simulator, name }) => simulator && name === deviceName);
    }
    const defaultSimulator = DEFAULT_SIMS[platformIdentifier || "com.apple.platform.iphonesimulator"];
 ...
 */
export async function install(device, app) {
 ...
 */
export function iosSpecificBuildFlags(params, args) {
    ...
    if (platform === "ios" || platform === "visionos") {
        ...
        if (destination === "device") {
            ...
            args.push("-sdk", sdk, "-destination", destination, "CODE_SIGNING_ALLOWED=NO");
            if (archs) {
 ...
 */
export async function launch(device, app) {
    const { simulator, identifier } = device;
    if (!simulator) {
        const launch = iosDeploy("--id", identifier, "--bundle", app, "--justlaunch", "--noinstall");
 ...
 */
export async function selectDevice(deviceNameOrPlatformIdentifier, deviceType, logger) {
    const devices = await getDevices();
    const [deviceName, platformIdentifier] = deviceNameOrPlatformIdentifier?.startsWith("com.apple.platform.")
    ...
    if (deviceType === "device") {
        const search = deviceName
            ...
            : ({ simulator, platform }) => !simulator && platform === platformIdentifier;
        ...
        if (!physicalDevice) {
            ...
            await new Promise((resolve) => prompt.question(`Failed to find ${device} — please make sure it is properly plugged in.\n\nPress any key to try again`, resolve));
            prompt.close();
    ...
    if (!device) {
        const foundDevices = devices
            .reduce((list, device) => {
            const { simulator, operatingSystemVersion, available, modelName } = device;
